...
export const calendarMonthDates = (monthDate) => {
  const [prevMonth, currentMonth, nextMonth] = [-1, 0, 1]
    .map((addMonths) => new Date(monthDate.getFullYear(), monthDate.getMonth() + addMonths));
  ...
  const monthDates = range(1, daysInMonth(currentMonth) + 1)
    .map((date) => new Date(currentMonth.getFullYear(), currentMonth.getMonth(), date));

  const lengthModulus = (prevPadding.length + monthDates.length) % 7;
  const nextPadding = range(lengthModulus ? Math.abs(lengthModulus - 7) : 0)
